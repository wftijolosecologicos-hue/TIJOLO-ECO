...
async function submitSoil(e){e.preventDefault();const form=e.currentTarget;const b=e.submitter;setBusy(b,true,'Cadastrando...');try{const f=Object.fromEntries(new FormData(form));if(num(f.sandPct)+num(f.clayPct)>105)throw new Error('A soma de areia e argila parece inconsistente.');const soil=await api('CREATE_SOIL',{soil:{name:f.name.trim(),origin:f.origin.trim(),sandPct:num(f.sandPct),clayPct:num(f.clayPct),notes:f.notes.trim()}});APP.soils.push(soil);saveCache();renderAll();form.reset();$('#soilDialog').close();toast('Terra cadastrada.');}catch(err){toast(err.message,true)}finally{setBusy(b,false)}}
async function submitPlan(e){e.preventDefault();const form=e.currentTarget;const b=e.submitter;setBusy(b,true,'Criando...');try{const f=Object.fromEntries(new FormData(form));const times=f.times.split(',').map(x=>x.trim()).filter(Boolean);if(times.some(x=>!/^([01]\d|2[0-3]):[0-5]\d$/.test(x)))throw new Error('Use horários no formato 07:00, 12:00.');const plan=await api('CREATE_PLAN',{plan:{name:f.name.trim(),days:num(f.days),times,description:f.description.trim()}});APP.plans.push(plan);saveCache();renderAll();form.reset();$('#planDialog').close();toast('Plano personalizado criado.');}catch(err){toast(err.message,true)}finally{setBusy(b,false)}}
...
async function changePassword(e){e.preventDefault();const form=e.currentTarget;const b=e.submitter,f=Object.fromEntries(new FormData(form));$('#passwordMessage').textContent='';if(f.newPassword!==f.confirmPassword){$('#passwordMessage').textContent='As novas senhas não coincidem.';return}setBusy(b,true,'Alterando...');try{const result=await api('CHANGE_CREDENTIALS',{newUsername:f.newUsername.trim(),newPassword:f.newPassword,verificationCode:f.verificationCode.trim()});APP.user.username=result.username;localStorage.setItem(STORE.user,JSON.stringify(APP.user));form.reset();$('#passwordMessage').style.color='var(--success)';$('#passwordMessage').textContent='Login e senha alterados com sucesso. Use o novo usuário no próximo acesso.';}catch(err){$('#passwordMessage').style.color='var(--danger)';$('#passwordMessage').textContent=err.message}finally{setBusy(b,false)}}
